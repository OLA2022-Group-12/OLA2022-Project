...
@given(
    ...
    seed=st.integers(min_value=0),
    num_customers=st.integers(min_value=10, max_value=40),
    budgets=st.lists(
...
@pytest.mark.xfail()  # Remove when test passing
def test_count_of_items_is_deterministically_equal_or_greater_with_bigger_budgets(
    ...
    second_interactions = get_day_of_interactions(
        rng, num_customers, second_budgets, env
    )
    ...
    assert len(first_interactions) <= len(second_interactions) + noise
    ...
    assert first_item_count <= secound_item_count + noise
